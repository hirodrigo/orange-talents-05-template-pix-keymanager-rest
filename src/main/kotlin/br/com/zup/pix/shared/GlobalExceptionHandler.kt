        ...
        val (httpStatus, message) = when (statusCode) {
            ...
            Status.ALREADY_EXISTS.code -> Pair(HttpStatus.UNPROCESSABLE_ENTITY, statusDescription)
                ...
                Pair(
                    ...
                    "Não foi possível completar a requisição devido ao erro: $statusDescription ($statusCode)"
                )
            }
        }

        return HttpResponse.status<Any?>(httpStatus).body(JsonError(message))
    }
}